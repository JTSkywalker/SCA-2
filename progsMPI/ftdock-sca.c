...
/************/
  ...
  for( rotation = first_rotation ; rotation <= 100/*SCA limit to 100. Otherwise it should be: Angles.n*/ ; rotation ++ ) {
	...
	electric_point_charge( Rotated_at_Origin_Mobile_Structure , grid_span , global_grid_size , mobile_elec_grid ) ;
      ...
      SCOREP_USER_REGION_DEFINE(ffft);
      SCOREP_USER_REGION_BEGIN(ffft, "ffft", SCOREP_USER_REGION_TYPE_COMMON);
      /* Forward Fourier Transforms */
      rfftwnd_one_real_to_complex( p , mobile_grid , NULL ) ;
      if( electrostatics == 1 ) {
	rfftwnd_one_real_to_complex( p , mobile_elec_grid , NULL ) ;
      }
      SCOREP_USER_REGION_END(ffft);
      ...
      */
      SCOREP_USER_REGION_DEFINE(convolution);
      SCOREP_USER_REGION_BEGIN(convolution, "convolution", SCOREP_USER_REGION_TYPE_COMMON);
	...
	for( fy = 0 ; fy < global_grid_size ; fy ++ ) {
      ...
      SCOREP_USER_REGION_END(convolution);

      SCOREP_USER_REGION_DEFINE(rfft);
      SCOREP_USER_REGION_BEGIN(rfft, "rfft", SCOREP_USER_REGION_TYPE_COMMON);
	...
	Scores[i].coord[3] = 0 ;
      ...
      SCOREP_USER_REGION_DEFINE(scoring);
      SCOREP_USER_REGION_BEGIN(scoring, "scoring", SCOREP_USER_REGION_TYPE_COMMON);
	...
	for( y = 0 ; y < global_grid_size ; y ++ ) {
	  ...
	  for( z = 0 ; z < global_grid_size ; z ++ ) {
		...
		// SCA - Don't remove this Straightinsertion sort.
      ...
      SCOREP_USER_REGION_END(scoring);

      for( i = 0 ; i < keep_per_rotation ; i ++ ) {

	if (max_score_value<Scores[i].score)
	  {
	    max_score_value = Scores[i].score ;
	    max_score = Scores[i];
	  }
      }

      SCOREP_USER_REGION_DEFINE(free_mem);
      SCOREP_USER_REGION_BEGIN(free_mem, "free_mem", SCOREP_USER_REGION_TYPE_COMMON);
	...
	free( Rotated_at_Origin_Mobile_Structure.Residue[i].Atom ) ;
      ...
      SCOREP_USER_REGION_END(free_mem);
  ...
  MPI_Finalize();
    ...
    printf("MAX Score value: %f at %4d %4d %4d \n",max_score_value, max_score.coord[1], max_score.coord[2], max_score.coord[3]);
